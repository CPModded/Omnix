  ...
  async (
    ...
    try {
      ...
      if (rawGuilds.length === 0 || Boolean((req.query as any).refresh)) {
        try {
          const tokenUser = await User.findOne({ discordId: (user as any).discordId })
            .select('+accessToken')
            .lean();
          const accessToken = (tokenUser as any)?.accessToken;
          if (accessToken) {
            ...
            if (Array.isArray(response.data)) {
              rawGuilds = response.data;
              await User.updateOne(
                { discordId: (user as any).discordId },
                { $set: { guilds: rawGuilds } }
              );
            }
          }
        } catch (refreshError) {
          console.warn('[Guilds] Rafraîchissement Discord impossible:', refreshError);
        }
      }
        ...
        rawGuilds
          .map((g: any) => {
            ...
            return normalizeGuild({
              ...g,
              memberCount: Number(live?.memberCount ?? g?.memberCount ?? 0),
        ...
        await Promise.all(
          guilds.map(
            async (guild) => {
              ...
              return {
              ...guild,
              memberCount: Number(liveGuild?.memberCount ?? guild.memberCount ?? 0),
