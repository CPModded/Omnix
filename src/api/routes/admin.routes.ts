...
router.get(
  ...
  async (
    ...
    try {
      ...
      const administrators = liveGuild ? [...liveGuild.members.cache.values()].filter((m:any) => !m.user.bot && (m.permissions?.has?.('Administrator') || m.id === liveGuild.ownerId)).slice(0,100).map((m:any)=>({id:m.id,username:m.user?.username||m.displayName||'Utilisateur',displayName:m.displayName,owner:m.id===liveGuild.ownerId,administrator:Boolean(m.permissions?.has?.('Administrator'))})) : [];
      const resultGuild = { ...config, description: liveGuild?.description || null, name: liveGuild?.name || config.name || 'Serveur Discord', icon: liveGuild?.iconURL?.({extension:'png',size:256}) || config.icon || null, memberCount:Number(liveGuild?.memberCount ?? (config as any).memberCount ?? 0), ownerId: liveGuild?.ownerId || (config as any).ownerId || null, botPresent:Boolean(liveGuild), blacklisted:Boolean((config as any).blacklisted), administrators };
