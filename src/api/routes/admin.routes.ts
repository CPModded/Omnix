...
router.post('/promos', requireStaff, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const promo = await PromoCode.create({code,discountType,discountValue,maxUses,usesCount:0,expiresAt,isActive:true});
