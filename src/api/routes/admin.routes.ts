...
router.patch('/control/activity', requireStaff, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    await writeAudit(req,{action:'update',category:'system',description:`Activité du robot modifiée: ${type} ${message}`,targetType:'SiteSettings',targetId:String(settings._id)});
    return res.json({success:true,data:settings});
