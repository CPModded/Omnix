...
router.post(
  ...
  async (
    ...
    try {
      ...
      if (userForLimit && !(await consumeAIMemoryAllowance(userForLimit, 1))) return res.status(429).json({ success:false, error:'Limite IA atteinte : 150 messages par heure.' });
