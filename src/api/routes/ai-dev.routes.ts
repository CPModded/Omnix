...
import { consumeAIMemoryAllowance } from '../../services/AiService';
...
router.post(
  ...
  async (
    ...
    try {
      ...
      const userForLimit = getAuthenticatedUserId(req);
      if (userForLimit && !(await consumeAIMemoryAllowance(userForLimit, 2))) return res.status(429).json({ success:false, error:'Limite IA atteinte : 150 messages par heure.' });
