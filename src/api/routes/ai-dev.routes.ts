...
`;
      ...
      const status = /\[OpenRouter\]\s*402/i.test(rawMessage) ? 402 : 500;
      const friendly = status === 402
        ? 'Le service IA a refusé la requête (402). Vérifie la clé OPENROUTER_API_KEY, son crédit/plan et OPENROUTER_MODEL dans les variables d’environnement.'
        : rawMessage;
      return res.status(status).json({
        success: false,
        code: status === 402 ? 'AI_PROVIDER_PAYMENT_REQUIRED' : 'AI_ERROR',
        error: friendly,
      });
    }
  }
