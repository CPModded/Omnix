...
export class GuildConfigController {
  ...
  static async updateConfig(req: AuthenticatedRequest, res: Response) {
    ...
    try {
      ...
      // La même fonction MongoDB est utilisée par le bot :
      // dashboard et bot lisent donc exactement la même configuration.
      const config = await updateGuildConfig(guildId, payload);
