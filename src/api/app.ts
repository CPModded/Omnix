...
async function requireAuthentication(
  ...
  /*
   * Compte suspendu : les pages web doivent afficher une page dédiée,
   * tandis que les routes API restent protégées par un 403 JSON.
   */
  try {
    ...
    if (dbUser?.isBlacklisted) {
      ...
      if (req.path !== '/suspended') {
        return res.redirect('/suspended');
      }
    }
  } catch (error) {
    console.error('[Auth] Vérification blacklist impossible :', error);
  }
...
app.get('/suspended', (req: Request, res: Response) => {
  try {
    ...
    if (!view) return res.status(403).send('Compte suspendu');
    return res.status(403).render('suspended');
  } catch {
    return res.status(403).send('Compte suspendu');
  }
});
