...
export async function consumeAIMemoryAllowance(userId: string, messageCount = 1): Promise<boolean> {
  ...
  try {
    // Reset an expired window atomically.
    const reset = await AiRateLimit.findOneAndUpdate(
      { key, $or: [{ windowStartedAt: { $lte: cutoff } }, { windowStartedAt: { $exists: false } }] },
      { $set: { windowStartedAt: now, count: requested } },
      { new: true },
    ).lean();
    if (reset) return reset.count <= AI_MEMORY_MESSAGES_PER_HOUR;

    // Consume only when the complete request fits inside the remaining allowance.
    const updated = await AiRateLimit.findOneAndUpdate(
      {
        key,
        windowStartedAt: { $gt: cutoff },
    ...
    // First request for this user. Duplicate-key races are handled below by a retry.
    try {
      const created = await AiRateLimit.create({ key, windowStartedAt: now, count: requested });
      return created.count <= AI_MEMORY_MESSAGES_PER_HOUR;
    } catch (error: any) {
      if (error?.code !== 11000) throw error;
      const retry = await AiRateLimit.findOneAndUpdate(
        { key, windowStartedAt: { $gt: cutoff }, count: { $lte: AI_MEMORY_MESSAGES_PER_HOUR - requested } },
        { $inc: { count: requested } },
        { new: true },
      ).lean();
