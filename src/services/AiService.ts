...
const aiMemoryUsage = new Map<string, number[]>();

/**
 * Consomme des emplacements de mémoire IA pour un utilisateur.
 * La limite est indépendante du serveur : elle est appliquée par utilisateur.
 */
export function consumeAIMemoryAllowance(userId: string, messageCount = 1): boolean {
  if (!userId || messageCount <= 0) return true;

  const now = Date.now();
  const current = (aiMemoryUsage.get(userId) ?? []).filter(
    timestamp => now - timestamp < AI_MEMORY_WINDOW_MS,
  );

  if (current.length + messageCount > AI_MEMORY_MESSAGES_PER_HOUR) {
    aiMemoryUsage.set(userId, current);
    return false;
  }

  for (let i = 0; i < messageCount; i++) current.push(now);
  aiMemoryUsage.set(userId, current);
  return true;
}
