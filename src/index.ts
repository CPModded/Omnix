...
async function connectDatabase(): Promise<void> {
  ...
  try {
    // Timeout court au démarrage : une panne DNS/SRV ne doit pas empêcher
    // le serveur HTTP Render de rester disponible.
    ...
    try {
      const cleanup = await User.updateMany(
        {},
        { $unset: { accessToken: '', refreshToken: '', tokenExpiresAt: '', licenses: '' } }
      );
      if (cleanup.modifiedCount > 0) {
        console.log(`[MongoDB] ✓ ${cleanup.modifiedCount} ancien(s) secret(s) OAuth supprimé(s).`);
      }
    } catch (cleanupError) {
      console.warn('[MongoDB] Nettoyage OAuth legacy impossible :', cleanupError);
    }
    ...
    console.error(
      '[MongoDB] ✗ Connexion impossible :',
      error?.message || error
    );
    console.warn('[MongoDB] OMNIX continue sans base de données. Nouvelle tentative dans 30 secondes.');
