...
import { processSecurityMessage } from '../utils/securityEngine';
import {
  getAIMemory,
...
async function execute(message: Message): Promise<void> {
  ...
  const blockedBySecurity = await processSecurityMessage(message);
  ...
  try {
    ...
    // Chaque requête utilisateur consomme 1 unité de quota.
    // Limite : 150 messages utilisateur par heure.
    if (!(await consumeAIMemoryAllowance(message.author.id, 1))) {
