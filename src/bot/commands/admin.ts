...
const adminCommand: Command = {
  ...
  async execute({
    ...
    if (!authorized) {
      if (
        ...
        return interaction.editReply({
          content:
            '❌ Accès refusé. Cette commande est strictement réservée au Staff OMNIX.',
        });
      }
    ...
    if (
      ...
      try {
        ...
        if (!userDb) {
          return interaction.editReply({
            content:
              "❌ Cet utilisateur ne s’est jamais connecté au Dashboard OMNIX.",
          });
        }
        ...
        if (action === 'remove') {
          ...
          return interaction.editReply({
            content:
              `✅ Premium retiré de **${target.username}**.`,
          });
        }

        /*
         * Attribution du Premium
         */
        ...
        return interaction.editReply({
          content:
            `✅ Premium accordé à **${target.username}**.\n\n` +
            `• Licence : \`${licenseKey}\`\n` +
        ...
        return interaction.editReply({
          content:
            `❌ Erreur lors de la modification du Premium : ${error?.message ?? 'Erreur inconnue'}`,
        });
      }
    }
    ...
    return interaction.reply({
      content:
        '❌ Sous-commande administrateur inconnue.',
      flags: 64,
    });
  },
};
